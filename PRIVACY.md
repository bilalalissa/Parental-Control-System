...
Stage 06D RC4's ad-hoc test installer stores one randomly generated 32-byte child endpoint private identity in a root-only local file. It contains no browsing data, account credential, address, message, schedule or person name; it never leaves the child except by signing the existing pairing protocol. The file persists across in-place upgrades and is deleted only by the documented child uninstaller. Stable Developer ID distribution must migrate this private identity to Keychain. A consumed-once installer maintenance marker contains only issue and expiry seconds and is removed at daemon startup.

Stage 06D browser-only enforcement retains adult-entered normalized domain rules plus bounded per-profile policy version, outcome and freshness. It does not retain blocked navigation history. Provisioning profiles, certificates, private keys, Team credentials, notarization secrets, device addresses, and physical-test logs must never enter the repository. The deferred system filter may not retain URLs, paths, queries, DNS or browsing history, packets, payloads, unrelated destinations, or unrelated process events. No network-content inspection is permitted.

Stage 06E adds at most 32 selected application rules containing application display name, bundle identifier, signing identifier and Team identifier. Enforcement does not require optional application-name sharing to remain enabled. The child reports only the matched bundle identifier, policy version, outcome (`quit-requested`, `closed`, or `session-locked`) and timestamp through the existing authenticated bounded queue. No executable path, command line, window/document title, file content or unrelated process event is transmitted or retained. Applying a newer empty policy removes all app-use rules.

...
5. **Browser extension boundary:** separately installed visible Chromium/Firefox extensions send bounded tab titles and query-free HTTP(S) origins through a fixed-origin native host. The host validates the exact signed parent browser path, signing identifier, and vendor Team ID; installed endpoint XPC separately validates the signed/root-protected host. Stage 06E adds an adult-enabled Safari local-test extension whose exact installed executable and package hash are authorized only for browser configuration/update XPC. Private tabs, page content, paths, queries, fragments, forms, cookies, passwords, and network traffic are excluded.
...
9. **Browser restriction boundary:** amended Stage 06D/06E sends validated domain rules through the signed LAN envelope to enrolled Chromium/Firefox profiles and an explicitly enabled Safari local-test profile. Browser declarative rules persist offline and are read back before acknowledgement. The parent shows profile-specific freshness and version, not universal coverage. Private/guest profiles and disabled extensions are explicit gaps. Safari production signing/publication, system-wide Network Extension and Endpoint Security work remain deferred.
10. **Windows service boundary:** Stage 07 installs one automatic x64 service and one visible ordinary-user WPF app. A length-prefixed named pipe is limited by an OS ACL and impersonated administrator check; only status and pairing exist. Machine identity is DPAPI-protected beneath a protected ProgramData ACL. The service advertises no monitoring, chat, action, or policy capability.
...
Stage 06D uses supported declarative WebExtension APIs under the developer's scope amendment, not Apple system extensions. Its local installer does not require managed entitlements. Stage 06E adds entitlement-free, post-launch app-use restriction through exact observed signing identities: the visible session helper requests normal termination and locks only if the app refuses. It does not claim Endpoint Security pre-launch denial or administrator resistance. WAN pause remains unavailable. Production browser signing/publication is a separate distribution gate. See [Stage 06D](../stages/stage-06d.md), [Stage 06E](../stages/stage-06e.md), and the deferred [ADR-0004](../adr/0004-macos-enforcement-extension-readiness.md).

Stage 08 extends the Stage 07 Windows foundation with documented WinEvent/window/process identity APIs, an active-session named-pipe boundary, a visible Chrome/Edge extension and origin-bound native host, protected bounded communication queues, and parent-controlled retention. It never collects window/page content, command lines, URL paths/queries, private tabs, or traffic. Windows schedules and enforcement remain later stages. See [Stage 08](../stages/stage-08.md).

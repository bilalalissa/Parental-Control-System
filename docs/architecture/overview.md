...
Stage 07 uses documented Windows service, WTS, DPAPI, named-pipe, registry, network, and WPF APIs. Its self-contained x64 MSI has no runtime cloud dependency and removes protected endpoint state only during explicit administrator uninstall. Windows application activity, browser integration, chat, schedules, and enforcement remain later stages. See [Stage 07](../stages/stage-07.md).

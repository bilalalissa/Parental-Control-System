...
| Deny selected website domains | Policy authoring is gated | **Stage 06D design only; blocked on Apple Network Extension entitlement and signed physical proof** | Not evaluated | Family Controls categories/tokens only, not this rule model | Separately managed restrictions only |
...
Stage 06D selects supported macOS Network Extension and Endpoint Security system-extension boundaries. Distribution is currently blocked because the signing Mac has no Developer ID identity or matching profiles, and Endpoint Security is an Apple-managed capability. Entitlement templates and a readiness checker do not make the controls operational. The UI and installer must not claim support until same-Team signed activation and the complete physical matrix pass.

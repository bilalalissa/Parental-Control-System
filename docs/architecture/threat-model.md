...
Stage-06E adds a deliberately weaker entitlement-free app-use boundary. It observes only ordinary running applications, validates an exact bundle/signing/Team identity, and asks a selected app to terminate after launch. It cannot deny execution before launch. An app can refuse or delay normal termination; the bounded fallback locks the child session once for that process and policy version, without force-killing it or entering a lock loop. Apple/system, login/recovery and parental-control processes are never eligible. A local administrator remains able to bypass or remove this control.
...
| Repeated app-restriction fallback | A refusing app causes a lock loop after each sign-in | Record the handled process and policy version; permit only one fallback until that process exits or a newer policy is accepted; regression-test the gate |
| App identity substitution | An unrelated or replaced binary is restricted, or a selected app update escapes silently | Require exact validated bundle, signing and Team identity; never match by display name/path; surface unavailable or changed identity for parent review |

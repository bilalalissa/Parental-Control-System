...
test("stage tracker uses an allowed state and identifies one active stage", async () => {
  ...
  assert.equal(active[0].branch, "stage/06c-router-wan-pause");
  assert.equal(active[0].version, "0.6.3-rc.1");
  assert.equal(active[0].status, "READY_FOR_DEVELOPER_TEST");
...
test("README and license identify pre-release status and terms", async () => {
  ...
  assert.match(readme, /STAGE-06C is ready for developer review; STAGE-07 has not begun/);

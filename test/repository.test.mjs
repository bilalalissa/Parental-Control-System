...
test("stage tracker uses an allowed state and identifies one active stage", async () => {
  ...
  assert.equal(active[0].branch, "stage/00-repository-foundation");
  assert.equal(active[0].version, "0.0.1-rc.1");
});
...
test("README and license identify pre-release status and terms", async () => {
  ...
  assert.match(readme, /Stage 00 \(architecture and contracts\)/);

...
### STAGE-06D — Managed browser website blocking (amended 2026-09-05)

The developer authorized `AUTHORIZE STAGE-06D SCOPE AMENDMENT: MANAGED BROWSER WEBSITE BLOCKING. PROCEED.` This replaces the implementation gate below for browser-only work. Implement one centrally managed domain policy through the authenticated local child host, Chromium and Firefox WebExtension packages sharing canonical source, bounded offline enforcement, per-browser/profile policy acknowledgements and coverage warnings. Browser collection remains optional and independent of blocking; private sessions and page/network content remain excluded. Safari, application launch denial, WAN pause, managed force-install, MDM, and Stage-07 are excluded. Do not claim all-browser coverage or resistance to extension removal, alternate/guest profiles, or administrators.

Produce a local macOS test installer and browser test packages with honest signing status. Production browser auto-updates must use supported store or signed self-distribution mechanisms; unpacked/temporary test installs are not automatic-update acceptance. External publisher registration, signing submissions, store publication, and managed deployment require separate approval and credentials. Keep the former system-extension design below as deferred historical scope, not as a prerequisite to the amended browser-only implementation.

#### Deferred system-wide enforcement design

Add three locally controlled macOS restrictions for a standard child account: deny explicitly selected nonessential application launches, deny explicitly selected website domains, and pause external Internet access for a bounded period while preserving the authenticated local Parent Controller connection and recovery traffic.

Gate implementation on supported Apple system extensions. Application launch authorization uses Endpoint Security and requires the Apple-granted `com.apple.developer.endpoint-security.client` managed entitlement. Domain and WAN filtering use a Network Extension content-filter system extension with `content-filter-provider-systemextension`. The containing child app requires the system-extension installation entitlement. All components must use one Team ID, explicit bundle identifiers, matching provisioning profiles, Developer ID signing for distribution, notarization, and visible system-extension activation/approval. Never invent, self-sign, or bypass a missing entitlement.

Application rules use a validated code identity and the standard child user, never only a display name or mutable path. Exclude the operating system, login/recovery, Parent Controller, child endpoint, installer/uninstaller, and essential accessibility/safety processes. Domain rules match normalized host/domain values only and collect no URL paths, queries, content, DNS history, or traffic payload. WAN pause defaults to a maximum of eight hours, covers IPv4 and IPv6, preserves same-LAN controller traffic plus proven DHCP/DNS/time/recovery needs, expires locally even if the controller disconnects, and fails to a visible recoverable state.

Before extension source or installer work, run an entitlement-readiness gate that validates a usable code-signing identity and matching host, Network Extension, and Endpoint Security provisioning profiles. If any prerequisite is absent, produce a source-only readiness dossier, mark the stage blocked, and do not expose controls or produce an enforcement installer. Creating Apple accounts, certificates, identifiers, entitlement requests, or profiles is external developer work; repository fixtures may contain only non-secret entitlement templates.

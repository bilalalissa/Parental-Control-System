...
- No public relay, hosted API, remote database, SaaS telemetry, Docker deployment, Kubernetes deployment, or mandatory account service may be introduced in stages 00–12. STAGE-06A is an explicitly authorized optional feasibility exception for documentation-only evaluation of manual third-party MDM enrollment; it must not make the core product depend on an external account, create an MDM account without separate approval, or transmit family data.
